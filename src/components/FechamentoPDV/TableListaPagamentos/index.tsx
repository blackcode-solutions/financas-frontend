...
interface TableListaPagamentosProps {
  ...
  selectedsPagamentos: any;
}

export function TableListaPagamentos({
  meusPagamentos,
  setTotaisPagamentos,
  setSelectedsPagamentos,
  selectedsPagamentos,
}: TableListaPagamentosProps) {
  const listaPagamentos = useListarPagamentos(
    meusPagamentos,
    setTotaisPagamentos
  );
  ...
  useEffect(() => {
    if (selectedsPagamentos) {
      if (setSelectedsPagamentos.length > 0) {
        const totalPagamentos = selectedsPagamentos.reduce(
          (acc: any, element: any) => {
