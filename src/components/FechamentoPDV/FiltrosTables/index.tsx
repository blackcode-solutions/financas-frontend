...
interface filtertablesProps {
  ...
  gridRef: any;
  setListarSelecionados: (value: boolean) => void;
  objFiltroUsuario: {
  ...
  return (
    <fieldset className={`fieldsetContainer ${styles.flex}`}>
      <div style={{ display: "flex", gap: "0.5rem" }}>
        ...
        <div className={styles.containerInput} style={{ width: "90px" }}>
          <label htmlFor="" className="label">
            N° Venda
          </label>
        ...
        <div className={styles.containerInput} style={{ width: "90px" }}>
          ...
          <InputComponent {...formulario.register("recebimentoVendaId")} />
        </div>
      ...
      >
        <SwitchComponent
          value={Cookies.get("usuarioIdEfinance")}
          onChange={(e) => {
            if (e.target.checked) {
              formulario.setValue("usuarioId", e.target.value);
            } else {
              formulario.setValue("usuarioId", "");
            }
          }}
        ...
        <SwitchComponent
          onChange={(e) => {
            if (e.target.checked) {
              setListarSelecionados(true);
            } else {
              setListarSelecionados(false);
            }
          }}
