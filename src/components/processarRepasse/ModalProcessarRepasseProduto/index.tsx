...
export function ModalProcessarRepassesProdutos({
  openModal,
  ...
  const [dataFim, setDataFim] = useState("");
    ...
    const processarRepasse = async () => {
      ...
      const objOptions = {
        dataInicio:
          convertToPtBrFormat(dataInicio), dataFim:convertToPtBrFormat(dataFim),tipoVenda:"PRODUTO"
      }
      // Listar itens para adicionar ao repasse
      const parcelasRecebimentoPDV:any = await fetchApiQuery("itensVendaProduto/listar", objOptions);
        ...
        for (const item of parcelasRecebimentoPDV?.parcelasRepasse) {
          await postGravarItensRepasse.mutateAsync({ ...item, repasseId,profissionalId:"0" });
        }
        close()
        Swal.fire({
                      position: "center",
  ...
  return (
    <Dialog
      ...
      modal
      header="Repasse de Produtos Empresa"
      draggable={false}
