...
export function useListarDetalhesRepasse({repasseId,openModal}: {repasseId:number,openModal:boolean}) {
  ...
  const fetchDetalhesRepasse = async (): Promise<any> => {
    const response = await fetchApiQuery<any>('repasseItem/listarDetalhesRecebimentosProdutos', { repasseId });
    return response;
  };
  ...
  const listaRepasses = Array.isArray(data?.itensRepasse) ? data?.itensRepasse : [];
